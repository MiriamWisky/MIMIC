CREATE
OR REPLACE TABLE tmp_target_condition
AS
SELECT co.condition_occurrence_id                    AS condition_occurrence_id,
       ...
       COALESCE(co.condition_end_date,
                dateadd(co.condition_start_date, 1)) AS condition_end_date
...
CREATE
OR REPLACE TABLE tmp_dates_un_condition
    AS
...
SELECT person_id                       AS person_id,
       condition_concept_id            AS condition_concept_id,
       dateadd(condition_end_date, 30) AS event_date,
       1                               AS event_type,
...
CREATE
OR REPLACE TABLE tmp_dates_rows_condition
AS
SELECT person_id            AS person_id,
       condition_concept_id AS condition_concept_id,
...
CREATE
OR REPLACE TABLE tmp_enddates_condition
AS
SELECT person_id                AS person_id,
       condition_concept_id     AS condition_concept_id,
...
CREATE
OR REPLACE TABLE tmp_conditionends
AS
...
SELECT hash(uuid())                         AS condition_era_id,
