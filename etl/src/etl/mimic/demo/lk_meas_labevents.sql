CREATE
OR REPLACE TABLE lk_meas_d_labitems_clean AS
...
CREATE
OR REPLACE TABLE lk_meas_labevents_clean AS
SELECT hash(uuid())   AS measurement_id,
    ...
    CASE
        WHEN substring(src.value,1, 1) IN ('<', '>')
                 AND substring(src.value,2, 1) = '='
            THEN substring(src.value,1, 2)
        WHEN substring(src.value,1, 1) IN ('=', '<', '>')
            THEN substring(src.value,1, 1)
...
-- -------------------------------------------------------------------
CREATE
OR REPLACE TABLE lk_meas_d_labitems_concept AS
...
CREATE
OR REPLACE TABLE lk_meas_labevents_hadm_id AS
...
CREATE
OR REPLACE TABLE lk_meas_labevents_mapped AS
