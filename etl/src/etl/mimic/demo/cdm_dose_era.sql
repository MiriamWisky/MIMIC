...
-- -------------------------------------------------------------------
CREATE
OR REPLACE TABLE tmp_drugIngredientExp AS
...
CREATE
OR REPLACE TABLE tmp_drugWithDose AS
...
CREATE
OR REPLACE TABLE tmp_cteDoseTarget AS
SELECT dwd.drug_exposure_id                                         AS drug_exposure_id,
       ...
       COALESCE(drug_exposure_end_date,
           ...
           -- return drug_exposure_end_date, otherwise go to next case
                NULLIF(date_add(drug_exposure_start_date, (1 * days_supply * (COALESCE(refills, 0) + 1))),
                       drug_exposure_start_date),
           --If days_supply != NULL or 0, return drug_exposure_start_date + days_supply,
           -- otherwise go to next case
                date_add(drug_exposure_start_date, 1)) AS drug_exposure_end_date
...
CREATE
OR REPLACE TABLE tmp_cteDoseEndDates_rawdata AS
...
CREATE
OR REPLACE TABLE tmp_cteDoseEndDates_e AS
SELECT person_id       AS person_id,
       drug_concept_id AS drug_concept_id,
       unit_concept_id AS unit_concept_id,
       dose_value      AS dose_value,
       ...
       MAX(start_ordinal) over (
        ...
        ORDER BY event_date, event_type ROWS UNBOUNDED PRECEDING)               AS start_ordinal, row_number() over (
        partition BY person_id, drug_concept_id, unit_concept_id, CAST(dose_value AS INTEGER)
...
CREATE
OR REPLACE TABLE tmp_cteDoseEndDates AS
SELECT person_id                             AS person_id,
       drug_concept_id                       AS drug_concept_id,
       unit_concept_id                       AS unit_concept_id,
       dose_value                            AS dose_value,
...
CREATE
OR REPLACE TABLE tmp_cteDoseFinalEnds AS
...
SELECT hash(uuid())                  AS dose_era_id,
       person_id                     AS person_id,
       drug_concept_id               AS drug_concept_id,
       unit_concept_id               AS unit_concept_id,
       dose_value                    AS dose_value,
