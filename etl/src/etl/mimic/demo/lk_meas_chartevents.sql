CREATE
OR REPLACE TABLE lk_chartevents_clean AS
SELECT src.subject_id             AS subject_id,
    ...
    IF(
        ...
        CAST(REGEXP_EXTRACT(src.value, r'([-]?[\d]+[.]?[\d]*)') AS NUMERIC),
        src.valuenum
    )                               AS valuenum,
    IF(
        REGEXP_EXTRACT(TRIM(src.value), r'(^[-]?[\d]+[.]?[\d]*[ ]*[a-z]+$)') IS NOT NULL, -- 7.5 mm etc
        TRIM(REGEXP_REPLACE(src.value, r'(\d+)', '')),
        src.valueuom
...
CREATE
OR REPLACE TABLE tmp_chartevents_code_dist AS
...
CREATE
OR REPLACE TABLE lk_chartevents_concept AS
...
CREATE
OR REPLACE TABLE lk_chartevents_mapped AS
...
CREATE
OR REPLACE TABLE lk_chartevents_condition_mapped AS
SELECT src.subject_id               AS subject_id,
       src.hadm_id                  AS hadm_id,
       src.stay_id                  AS stay_id,
       src.start_datetime           AS start_datetime,
